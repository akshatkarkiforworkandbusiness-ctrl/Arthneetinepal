    ...
    >
      <header className="mb-24 flex flex-col md:flex-row justify-between items-end gap-12">
        <div className="max-w-3xl">
          <span className="text-[10px] font-black text-brand-emerald mb-4 block uppercase tracking-[0.4em]">INSTITUTIONAL ARCHIVE</span>
        ...
        <div className="flex gap-4">
          <button 
            ...
            className="px-8 py-5 border-2 border-brand-emerald text-brand-emerald rounded-lg-2xl text-[10px] font-black uppercase tracking-widest hover:bg-brand-emerald hover:text-white transition-all"
          >
            SUBMIT MANUSCRIPT
          </button>
        </div>
      ...
      <section className="space-y-12 mb-32">
        {archive.length > 0 ? archive.map((paper, i) => (
          <motion.article 
            ...
            className="bg-white rounded-lg-[60px] border border-white/10 overflow-hidden flex flex-col lg:flex-row shadow-sm hover:shadow-2xl transition-all duration-700 group"
          >
            ...
            <div className="lg:w-3/5 p-12 md:p-20 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-center mb-10">
                  <Badge variant="outline" className="px-5 py-2 bg-brand-emerald/10 text-brand-emerald border-transparent rounded-lg text-[10px] font-black uppercase tracking-widest">
                    {paper.category}
                  </Badge>
                  <span className="text-[10px] font-bold text-white/30 uppercase tracking-widest">{paper.date}</span>
                </div>
                <h3 className="font-sans tracking-tight font-semibold text-5xl text-white italic mb-8 leading-tight group-hover:text-brand-emerald transition-colors tracking-tight">
                  {paper.title}
                </h3>
                <p className="text-lg text-white/60 italic leading-relaxed mb-12">
                  "{paper.abstract}"
                </p>
              </div>
              <div className="flex items-center justify-between border-t border-white/5 pt-10">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-lg bg-surface-base border border-surface-high flex items-center justify-center font-bold text-xs text-brand-emerald">
                    {paper.author[0]}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-white">{paper.author}</p>
                    <p className="text-[10px] text-white/40 uppercase font-black tracking-widest">Fellow</p>
                  </div>
                </div>
                <button className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-white/40 hover:text-brand-emerald transition-all">
                  <Download size={20} />
                  DOWNLOAD PDF
                </button>
              </div>
          ...
          <div className="text-center py-32 bg-white rounded-lg-[60px] border border-dashed border-brand-emerald/20">
            <p className="text-brand-emerald/40 italic">The archive is currently empty.</p>
          </div>
        )}
      </section>

      <section className="bg-white text-white p-16 md:p-32 rounded-lg-[80px] relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          <div>
            ...
            <div className="flex gap-8">
              <div className="text-center">
                <p className="text-4xl font-sans tracking-tight font-semibold italic mb-2 text-brand-emerald">42+</p>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/40">Papers Published</p>
              </div>
              <div className="text-center border-l border-white/10 pl-8">
                <p className="text-4xl font-sans tracking-tight font-semibold italic mb-2 text-brand-emerald-light">1.2k</p>
          ...
          <div className="bg-white/5 backdrop-blur-2xl p-12 rounded-lg-[48px] border border-white/10">
            <span className="text-[10px] font-black text-brand-emerald mb-6 block uppercase tracking-widest">SUBMISSION PORTAL</span>
            ...
            <button 
              onClick={() => setShowSubmitModal(true)}
              className="w-full bg-text-primary text-surface-base py-5 rounded-lg-2xl text-[10px] font-black uppercase tracking-[0.3em] hover:bg-brand-emerald-light hover:text-white transition-all"
            >
              START SUBMISSION
            </button>
          </div>
      ...
      <AnimatePresence>
        {showSubmitModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 md:p-8">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowSubmitModal(false)} className="absolute inset-0 bg-white/40 backdrop-blur-md" />
              ...
              <form onSubmit={handleSubmitManuscript} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input required type="text" value={manuscript.name} onChange={e => setManuscript({...manuscript, name: e.target.value})} className="w-full bg-surface-base border border-surface-high rounded-lg p-4 text-sm text-text-primary outline-none focus:border-brand-emerald transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-text-muted">Institution</label>
                    <input required type="text" value={manuscript.institution} onChange={e => setManuscript({...manuscript, institution: e.target.value})} className="w-full bg-surface-base border border-surface-high rounded-lg p-4 text-sm text-text-primary outline-none focus:border-brand-emerald transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-text-muted">Email Address</label>
                    <input required type="email" value={manuscript.email} onChange={e => setManuscript({...manuscript, email: e.target.value})} className="w-full bg-surface-base border border-surface-high rounded-lg p-4 text-sm text-text-primary outline-none focus:border-brand-emerald transition-all" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <textarea required value={manuscript.content} onChange={e => setManuscript({...manuscript, content: e.target.value})} className="w-full bg-surface-base border border-surface-high rounded-lg p-4 text-sm text-text-primary outline-none h-40 resize-none focus:border-brand-emerald transition-all" />
                </div>
                <button 
                  type="submit" 
                  disabled={isUploading}
                  className="w-full bg-brand-emerald text-white py-4 rounded-lg-2xl text-[10px] font-black uppercase tracking-[0.3em] hover:bg-brand-emerald-light transition-all shadow-xl disabled:opacity-30"
                >
      ...
      <AnimatePresence>
        {showRigorModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowRigorModal(false)} className="absolute inset-0 bg-white/40 backdrop-blur-md" />
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-surface-raised p-8 rounded-lg-[40px] max-w-lg w-full shadow-2xl relative">
              ...
              <form onSubmit={handleAddRigor} className="space-y-6">
                <textarea required value={newRigorText} onChange={e => setNewRigorText(e.target.value)} className="w-full bg-surface-base border border-surface-high rounded-lg p-4 text-sm text-text-primary outline-none h-32 resize-none focus:border-brand-emerald transition-all" placeholder="Statement about academic standards..." />
                <button type="submit" className="w-full bg-brand-emerald text-white py-4 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-brand-emerald-light transition-all">ADD STATEMENT</button>
              </form>
            </motion.div>
          </div>
        )}
