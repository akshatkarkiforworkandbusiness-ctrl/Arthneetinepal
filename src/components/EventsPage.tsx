...
export default function EventsPage() {
  const { user } = useAuth();
  const isAdmin = user?.email === 'karkiakshat208011@gmail.com';
              ...
              >
                ...
                <div className="flex-1 space-y-6">
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-black uppercase tracking-widest text-crimson bg-crimson/10 px-3 py-1 rounded">
                      {event.category}
                    </span>
                    {isAdmin && (
