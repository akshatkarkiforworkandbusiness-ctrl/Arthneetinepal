    ...
    <group ref={group} position={[0, 0.2, 0]}>
      {bars.map((bar, i) => (
        <mesh key={i} position={[bar.x, bar.h / 2 - 0.4, 0]}>
          ...
          <meshStandardMaterial color={color} transparent opacity={0.8} emissive={color} emissiveIntensity={0.2} roughness={0.3} metalness={0.4} />
        </mesh>
      ))}
      ...
      {candles.map((c, i) => (
        <group key={i} position={[c.x, 0, 0]}>
          <mesh position={[0, 0, 0]}>
            ...
            <meshStandardMaterial color={c.green ? '#00875a' : '#ef4444'} transparent opacity={0.7} />
          </mesh>
  ...
  return (
    <group ref={groupRef} position={[0, 0.1, 0]}>
      ...
      <mesh rotation={[Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color="#FFFFFF" transparent opacity={0.5} emissive={color} emissiveIntensity={0.2} />
      </mesh>
...
function PieChart({ color }: { color: string }) {
  ...
  const segments = useMemo(() => {
    const slices = [
      ...
      { start: 0.35, end: 0.6, color: '#2563eb' },
      { start: 0.6, end: 0.8, color: '#9333ea' },
    ...
    <group ref={group} position={[0, 0.1, 0]} rotation={[0.3, 0, 0]}>
      {segments.map((seg, i) => (
        <mesh key={i}>
          ...
          <meshStandardMaterial color={seg.color} transparent opacity={0.8} roughness={0.3} metalness={0.4} emissive={seg.color} emissiveIntensity={0.2} />
        </mesh>
      ))}
    </group>
  );
  ...
  return (
    <group ref={group} position={[0, 0, 0]}>
      <mesh geometry={tubeGeo}>
        <meshStandardMaterial color={color} emissive={color} emissiveIntensity={0.3} roughness={0.2} metalness={0.5} />
      </mesh>
      <mesh position={[0.7, 0.55, 0]} rotation={[0, 0, Math.PI / 4]}>
        <coneGeometry args={[0.08, 0.18, 4]} />
        <meshStandardMaterial color={color} emissive={color} emissiveIntensity={0.4} roughness={0.2} metalness={0.5} />
      </mesh>
    </group>
  );
}
  ...
  return (
    <group ref={coinRef} position={[0, 0.1, 0]}>
      <mesh>
        ...
        <meshStandardMaterial color={color} transparent opacity={0.7} roughness={0.2} metalness={0.6} emissive={color} emissiveIntensity={0.3} />
      </mesh>
      <mesh>
...
function OrbitParticles({ color, count = 18 }: { color: string; count?: number }) {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial color={color} size={0.035} transparent opacity={0.7} sizeAttenuation depthWrite={false} />
    </points>
  );
}
...
export default function CardScene3D({ color, category }: { color: string; category: string }) {
  ...
  return (
    <div className="absolute inset-0 z-0">
      <Canvas camera={{ position: [0, 0, 2.8], fov: 45 }} gl={{ alpha: true, antialias: true }} dpr={[1, 1.5]}>
        <ambientLight intensity={1.2} />
        <pointLight position={[2, 2, 2]} color={color} intensity={2} distance={8} />
        <pointLight position={[-2, -1, 1]} color="#ffffff" intensity={1} distance={6} />
