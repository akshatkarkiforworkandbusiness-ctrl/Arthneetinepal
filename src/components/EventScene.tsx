...
function Particles({ count = 80, color = '#00875a' }: { count?: number; color?: string }) {
  ...
  return (
    <>
      <pointLight ref={light} color={color} intensity={2.5} distance={15} />
      <points ref={mesh}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" count={count} array={positions} itemSize={3} />
        </bufferGeometry>
        <pointsMaterial color={color} size={0.06} transparent opacity={0.65} sizeAttenuation depthWrite={false} />
      </points>
    </>
  );
}
  ...
  return (
    <div className={`absolute inset-0 pointer-events-none ${className}`} style={{ zIndex: 0 }}>
      <Canvas camera={{ position: [0, 0, 6], fov: 50 }} gl={{ alpha: true, antialias: false }} dpr={[1, 1.5]}>
        ...
        <Particles count={60} color="#00875a" />
        <Particles count={30} color="#00F59B" />
